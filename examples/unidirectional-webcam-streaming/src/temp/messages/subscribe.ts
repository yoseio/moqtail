...
import { deserializeParams, type ParameterTemp, serializeParams } from "../utils/parameter";
import { concatBuffer, numberToVarInt, stringToVarBytes, varIntToNumber, varBytesToString, setUint8, getUint8 } from "../utils/bytes";

export const serializeSubscribe = (props: { subscribeId: number, trackAlias: number, namespace: string[], trackName: string, subscriberPriority: number, groupOrder: GROUP_ORDER, filterType: SUBSCRIBE_FILTER, startGroup?: number, startObject?: number, endGroup?: number, parameters?: ParameterTemp[] }) => {
