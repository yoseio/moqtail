...
class MoQTCommunicator {
    ...
    async closeSubgroupStreams(subgroupIds) {
        subgroupIds.map(id => {
            this.closeSubgroupStream(id);
        });
    }
    async createSubgroupStream({ subgroupId, subgroupHeader }) {
        if (this.state === COMMUNICATOR_STATE.STOPPED) {
    ...
    async readSubgroupObject(reader, trackAlias) {
        let done = false;
        while (!done) {
    ...
    async readDatagramObject(reader) {
