...
import { CONTROL_MESSAGE, deserializeVideoDecoderConfig, LOC_EXTENSION_HEADER_TYPE, MOQT_DRAFT08_VERSION, MOQT_DRAFT09_VERSION, MOQT_DRAFT10_VERSION, serializeClientSetup, serializeSubscribe, STREAM, deserializeAudioDecoderConfig } from "../temp";
// @ts-ignore
...
export class Subscriber {
    constructor(props) {
        ...
        this.audioWaitingForKeyFrame = true;
    ...
    getSubscriptionByTrackAlias(trackAlias) {
    ...
    generateReadableStreamFromBuffer(value) {
    ...
    audioDataToAudioBuffer(audioData) {
        const audioBuffer = this.audioCtx.createBuffer(audioData.numberOfChannels, audioData.numberOfFrames, audioData.sampleRate);
    ...
    communicatorMessageHandler(message) {
        ...
        switch (message.data.type) {
            ...
            case `stream-${STREAM.SUBGROUP_HEADER}`:
                ...
                break;
            case 'subgroupObject':
                const encodedChunkInit = message.data.data.encodedChunkInit;
                ...
                const videoTrackAlias = message.data.data.trackAlias;
                sub = this.subscription.find(s => s.subscribe.trackAlias === videoTrackAlias);
                const header = message.data.data.header;
                ...
                header.extensionHeaders.map(h => {
                    ...
                    const readableStream = this.generateReadableStreamFromBuffer(h.value);
            ...
            case 'subgroupObjectStatus':
                ...
                break;
            case 'datagramObject':
                const datagramObject = message.data.data;
                ...
                datagramObject.header.extensionHeaders.map(h => {
                    if (h.id !== LOC_EXTENSION_HEADER_TYPE.AUDIO_CONFIG)
                        return;
                    const readableStream = this.generateReadableStreamFromBuffer(h.value);
